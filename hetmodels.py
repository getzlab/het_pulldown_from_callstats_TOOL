...
def run_snp_mixture_model(nref, nalt, alpha=None, nit=50):
    ...
    p_df = pd.DataFrame(snp_post.T,columns = ['prob_het','prob_homref','prob_homalt','prob_other'])
...
def method_mixture_Eloss(snp_post,nref,nalt,log_epsilon,skew,pi,min_lik=1e-50):
    ...
    lik = method_mixture_likelihood(nref,nalt,np.exp(log_epsilon),skew)
    lik = np.maximum(min_lik,lik)
    lik = lik.reshape(4, lik.shape[1])
